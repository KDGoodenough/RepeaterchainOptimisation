...
labels = ['200 km', '400 km', '600 km', '800 km']
...
fig, axes = plt.subplots(nrows=3, ncols=3, sharex = True, sharey = True, figsize = (7,5))
...
select_and_generate_plots(colors = colors, labels = labels, linestyles = linestyles, ax=axes[2][0])
select_and_generate_plots(colors = colors, labels = labels, linestyles = linestyles, ax=axes[2][1])
select_and_generate_plots(colors = colors, labels = labels, linestyles = linestyles, ax=axes[2][2])


axes[0][1].legend(handles=patch2
                  , prop={'size': 0.5}, bbox_to_anchor=(0.6, 1.115),
         ncol=1, loc = 'center', handletextpad=0.4, framealpha=0)

lgd = fig.legend(handles=patches, prop={'size': 12.5}, bbox_to_anchor=(0.54, 0.925),
         ncol=4, loc = 'center', handletextpad=0.4)


#axes[0][1].get_legend().remove()
#axes[0][1].add_artist(lgd)
name = 'Notes/figs/test12.pdf'
...
axes[1][0].set_ylabel('Generation time (s)', fontsize=12)
axes[2][1].set_xlabel('Fidelity', fontsize=12)
